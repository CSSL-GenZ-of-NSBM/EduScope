...
import { Faculty, AffiliatedUniversity, DegreeModule, Degree } from "@/types"

export default function AdminEditDegreePage() {
  ...
  const fetchDegree = async () => {
    ...
    try {
      ...
      if (data.success) {
        const degree: Degree = data.data
        setFormData({
          degreeName: degree.degreeName,
          faculty: degree.faculty,
          affiliatedUniversity: degree.affiliatedUniversity,
          duration: degree.duration.toString(),
          price: degree.price.toString(),
          description: degree.description || "",
          admissionRequirements: (degree.admissionRequirements && degree.admissionRequirements.length > 0) ? degree.admissionRequirements : [""],
          careerPaths: (degree.careerPaths && degree.careerPaths.length > 0) ? degree.careerPaths : [""],
          isActive: degree.isActive
        })
        setModules(degree.modules || [])
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setTimeout(() => {
          router.push(`/admin/degrees/${degreeId}`)
        }, 2000)
      } else {
        setError(data.error || "Failed to update degree")
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Button variant="outline" onClick={() => router.push(`/admin/degrees/${degreeId}`)}>
          <ArrowLeft className="w-4 h-4 mr-2" />
          Back to Degree
        </Button>
        <div>
          <h2 className="text-3xl font-bold text-gray-900">Edit Degree</h2>
          <p className="text-gray-600">Update degree programme information</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <CardHeader>
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <CardDescription>Update the basic details of the degree programme</CardDescription>
              </div>
            </div>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Update the admission requirements for this degree</CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Update potential career paths for graduates</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>Update modules/courses for each year and semester</CardDescription>
              </div>
              <Button
                type="button"
                variant="outline"
        ...
        <div className="flex justify-end gap-4 pt-6">
          <Button
            type="button"
            variant="outline"
            onClick={() => router.push(`/admin/degrees/${degreeId}`)}
            disabled={saving}
          >
            Cancel
          </Button>
